  ...
  return (
    ...
    {Array.from({ length: 5 }).map((_, index) => (
          <div className="animate-pulse"  key={index}>
            ...
            <div className="h-2 bg-[#343536] rounded-full max-w-[360px] mb-5"></div>
          </div>
    ))
    }
    </div>
            ...
            <>
              ...
              {results.map((result, index) => (
                <div key={index} className="mb-8">
                  <div className="text-sm text-[#909296] mb-1">{result.link}</div>
                  <h2 className="text-xl mb-1">
                    <a href={result.link} className="text-[#8AB4F8] hover:underline" target="_blank" rel="noopener noreferrer">
                      {result.title}
                    </a>
                  </h2>
